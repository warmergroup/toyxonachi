...
<!--import { useGetTariflarQuery } from "~/api/getTariflar";-->
...
<!--const selectedTarif = ref<Record<string, any> | null>(null);-->
...
<!--    // Agar store'da toyxona mavjud bo'lmasa, API'dan yuklash-->
<!--    if (!toyxona.value) {-->
<!--      console.log('Toyxona store\'da topilmadi, API\'dan yuklanmoqda...');-->

<!--      try {-->
<!--        // API'dan to'yxona ma'lumotlarini olish-->
<!--        const { data } = await $axios.get('/wedding-halls/show', { params: { id } });-->

<!--        if (data && data.data) {-->
<!--          // To'yxonani store'ga qo'shish-->
<!--          toyxonalarStore.addToyxona(data.data);-->
<!--          console.log('Toyxona ma\'lumotlari API\'dan yuklandi va store\'ga qo\'shildi');-->
<!--        } else {-->
<!--          const err = new Error(t('venue.notFound'));-->
<!--          toyxonalarStore.setError(err);-->
<!--          error.value = t('venue.notFound');-->
<!--        }-->
<!--      } catch (e) {-->
<!--        console.error('Toyxona API yuklashda xatolik:', e);-->
<!--        error.value = t('common.error');-->
<!--        toyxonalarStore.setError(e instanceof Error ? e : new Error(String(e)));-->
<!--      }-->
<!--    } else {-->
<!--      console.log('Toyxona store\'dan olindi:', toyxona.value.name);-->
<!--    }-->
<!--  } catch (e) {-->
<!--    console.error('Toyxona sahifasi xatolik:', e);-->
<!--    error.value = t('common.error');-->
<!--  } finally {-->
...
<!--        </template>-->
<!--        &lt;!&ndash; <div v-if="!isLargeScreen" class="fixed top-0 left-0 right-0 w-full h-auto">-->
<!--          <UiCarousel :items="toyxona.images || []" />-->
<!--        </div> &ndash;&gt;-->
<!--        <div v-if="!isLargeScreen" ref="imageRef" class="sticky top-0 w-full h-auto">-->
<!--          <div class="relative">-->
<!--            <UiCarousel-->
<!--              v-if="toyxona.wedding_hall_pictures && Array.isArray(toyxona.wedding_hall_pictures) && toyxona.wedding_hall_pictures.length > 1"-->
<!--              :items="toyxona.wedding_hall_pictures.filter(pic => pic && pic.image_url).map(pic => `https://toyxonachi.uz/storage/images/${pic.image_url}`)"-->
<!--              :rounded="false" :arrows="false" />-->
<!--            <NuxtImg v-else-if="toyxona.wedding_hall_pictures && toyxona.wedding_hall_pictures.length > 0"-->
<!--              :src="`https://toyxonachi.uz/storage/images/${toyxona.wedding_hall_pictures[0].image_url}`"-->
...
<!--          <UiTarifDrawer v-if="selectedTarif" :open="isDrawerOpen" :tarif="selectedTarif"-->
<!--            :component-type="'tarif' as ComponentType" @update:open="isDrawerOpen = $event"-->
...
<!--              class="w-full h-48" />-->
<!--            &lt;!&ndash; {{ toyxona.latitude }}, {{ toyxona.longitude }} &ndash;&gt;-->
<!--          </div>-->
