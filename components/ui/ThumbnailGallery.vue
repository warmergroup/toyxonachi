...
const images = computed(() => props.toyxona.images || [])
</script>
    ...
    <div class="relative w-full rounded-lg overflow-hidden min-h-[200px] flex items-center justify-center">
      ...
      <div v-if="images.length > 0"
        ...
        <div class="flex gap-2 items-center">
          <h1 class="text-3xl font-bold text-white">{{ props.toyxona.name }}</h1>
        </div>
        <div class="flex items-center">
          <div class="text-white flex gap-5">
            <span>{{ props.toyxona.address || 'Manzil kiritilmagan' }}</span>
          </div>
        </div>
      </div>
    </div>
