  ...
  <div class="flex gap-2">
    ...
    <div v-if="images.length > 1" class="flex flex-col gap-2">
      <div
        ...
        <NuxtImg
          :src="image" :alt="`image ${index + 1}`" loading="lazy" class="w-full h-full object-cover"
          placeholder/>
      </div>
    </div>
    ...
    <div class="relative w-full rounded-lg overflow-hidden min-h-[200px] flex items-center justify-center">
      <template v-if="images.length > 0">
        <NuxtImg
          :src="selectedImage || images[0]" :alt="props.toyxona.name" class="w-full aspect-video object-cover"
